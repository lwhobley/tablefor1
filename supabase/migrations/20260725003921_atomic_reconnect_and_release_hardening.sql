-- Schedule a reconnect dinner as one transaction. The row lock prevents both
-- diners from racing to create duplicate events, and any failure rolls back
-- the event, bookings, match, and request update together.
...
REVOKE EXECUTE ON FUNCTION public.create_reconnect_dinner_atomic(uuid, uuid, timestamptz)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_reconnect_dinner_atomic(uuid, uuid, timestamptz)
  TO authenticated;
